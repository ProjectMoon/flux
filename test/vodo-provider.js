...
vows.describe('Flux Core (VODO provider)').addBatch({
	'flux.search': {
		topic: function() {
			flux.search('zenith', this.callback);
		},
	...
	'flux.locate (single torrent)': {
		topic: function() {
			var self = this;
			flux.search('zenith', function(err, torrents) {
	...
	'flux.source': {
		topic: function() {
			var self = this;
			flux.search('zenith', function(err, torrents) {
				flux.source('vodo.net', torrents[0], self.callback);
			});
		},
